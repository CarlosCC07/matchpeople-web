...
<head>
	<meta charset="UTF-8">
	<title>Matchpeople - Borrador Vacantes</title>
...
<?php include("menus/menu_vacantes.php");?>
  ...
  <div class="ochenta centar" >
    <?php
	 ...
	 $sql = "SELECT id, vacante, ciudad, fecha, Nombre_empresa, des_corta, estatusId FROM listavacantes where estatusId = 2 AND papeleria != 1";
  			...
  			if (mysqli_num_rows($result) > 0) {
  				...
  				while($row = mysqli_fetch_assoc($result)) {
					...
					<?php } } else {	echo "<h3>No hay vacantes en borrador</h3>";}	mysqli_close($con); ?>
  </div>
</div>
