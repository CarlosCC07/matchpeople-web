...
if(isset($_POST['import_data'])){
 ...
 $file_mimes = array('text/x-comma-separated-values', 'text/comma-separated-values', 'application/octet-stream', 'application/vnd.ms-excel', 'application/x-csv', 'text/x-csv', 'text/csv', 'application/csv', 'application/excel', 'application/vnd.msexcel', 'text/plain');
        ...
        if(is_uploaded_file($_FILES['file']['tmp_name'])){
          ...
          // get data records from csv file
            ...
            if(mysqli_num_rows($resultset)) {
              ...
              Nombre ='".utf8_encode ($emp_record[2])."',
              FechaIngreso ='".utf8_encode ($emp_record[3])."',
              Departamento ='".utf8_encode ($emp_record[4])."',
              Clave ='".utf8_encode ($emp_record[5])."',
              Puesto ='".utf8_encode ($emp_record[6])."',
              Planta ='".utf8_encode ($emp_record[7])."',
              Segmento ='".utf8_encode ($emp_record[8])."',
              Turno ='".utf8_encode ($emp_record[9])."',
              FechaNacimiento ='".utf8_encode ($emp_record[10])."',
              Grupo ='".utf8_encode ($emp_record[11])."',
              Email ='".utf8_encode ($emp_record[12])."'
              ...
              (IdPersonal,IdEmpresa, NumNomina,
              Nombre, FechaIngreso,
              Departamento, Clave,
              Puesto, Planta,
              Segmento, Turno,
              ...
              ( '$emp_record[0]',
                '$ide',
                '$emp_record[1]',
                '".utf8_encode($emp_record[2])."',
                '$emp_record[3]',
                '".utf8_encode($emp_record[4])."',
                '$emp_record[5]',
                '".utf8_encode($emp_record[6])."',
                '".utf8_encode($emp_record[7])."',
                '".utf8_encode($emp_record[8])."',
                '".utf8_encode($emp_record[9])."',
                '$emp_record[10]',
                '".utf8_encode($emp_record[11])."',
                '".utf8_encode($emp_record[12])."',
                '$ClaveTiempo',
                '$ClaveTiempo')";
              mysqli_query($con, $mysql_insert) or die("database error:". mysqli_error($con));

              $mysql_insert = "INSERT INTO resencuesta_sociometria(
                IdResEncuesta,IdEmpresa, IdPersonal,
    ...
    $sql="DELETE from resencuesta_sociometria WHERE ClaveTiempo = $ClaveTiempo";
    $resultset = mysqli_query($con, $sql) or die("database error:". mysqli_error($con));

    $sql="DELETE from personal_sociometria WHERE ClaveTiempo = $ClaveTiempo";
    $resultset = mysqli_query($con, $sql) or die("database error:". mysqli_error($con));
  }
  header("Location:personal_sociometria.php?id=$ide".$import_status);
  ?>
