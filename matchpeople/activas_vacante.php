...
<head>
	<meta charset="UTF-8">
	<title>Matchpeople - Vacantes Activas</title>
...
<?php include("menus/menu_vacantes.php");?>
  ...
  <div class="ochenta centar" >
    <?php
	 ...
	 $sql = "SELECT id, vacante, ciudad, fecha, Nombre_empresa, des_corta, estatusId FROM listavacantes where papeleria != 1 AND estatusId = 1";
  			...
  			if (mysqli_num_rows($result) > 0) {
  				...
  				echo "<form class='box_vacante' action='editar_vacante.php?id=".$row['id']."' enctype='multipart/form-data' method='POST'>";?>
					...
					<?php } } else {	echo "<h3>No hay vacantes activas</h3>";}	mysqli_close($con); ?>
  </div>
</div>
