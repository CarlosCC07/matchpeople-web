...
if($_FILES["upload_img"]["name"]){
    ...
    if(!$_FILES["upload_img"]["error"]){
        $archivo = $_FILES["upload_img"]["name"];
        ...
        if(pathinfo($archivo, PATHINFO_EXTENSION)=="png" || pathinfo($archivo, PATHINFO_EXTENSION)=="jpg"){
            $sql = "INSERT INTO logocliente ( empresa, logo) VALUES ('".utf8_decode($_POST['empresa'])."','$archivo')";
            //Si hay un error en la transacción.
            if(!mysqli_query($con, $sql)){ die('Error: ' . mysqli_error($con));
            //Si todo se realizó correctamente se sube el archivo al servidor.
            }else{
                move_uploaded_file($_FILES['upload_img']['tmp_name'], '../img/logosClientes/'.$archivo);
